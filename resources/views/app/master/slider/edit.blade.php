...
@push('js')
    ...
    <script>
        $(function() {
            ...
            $('#desc_short').val(@json($slider?->desc_short))
            $('#komposisi').val(@json($slider?->komposisi))
            AutoNumeric.getAutoNumericElement('#harga').set(@json($slider?->harga))
            $('#desc_long').html(@json($slider?->desc_long))
